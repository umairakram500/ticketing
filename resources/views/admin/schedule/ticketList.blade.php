            ...
            <div class="panel">
                <div class="panel-body p-0">
                    <table class="table table-bordered table-striped">
                        ...
                        @forelse($schedule->tickets  as $i => $ticket)
                            ...
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-danger" data-message="Are Soue you want to cancel this Ticket" data-delete="{{ route('admin.ticket.destroy', [$schedule->id, $ticket->id]) }}">Cancel</button>
                                    @if(!$ticket->paid)
                                    <button class="btn btn-success" onclick="paidTicket('{{ route('admin.ticket.paid', $ticket->id) }}', this)">Paid</button>
                                    @else
                                    <a href="{{ route('admin.ticket.show', [$ticket->schedule_id, $ticket->id]) }}" class="btn btn-success" onclick="paidTicket('{{ route('admin.ticket.paid', $ticket->id) }}', this)">Print</a>
                                    @endif
                                </td>
                            </tr>
                        @empty
